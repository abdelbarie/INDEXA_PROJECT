...
const QuestionBox = (props) => {
    ...
    return (
        <div className={`QAbox grid grid-cols-12 w-full gap-1 md:w-8/12 lg:w-7/12 mx-auto my-2 md:my-4 border-b-4 questionBox py-2 justify-between`}>
            ...
            <div className={ `${locale === 'fr' ? 'text-left' : 'text-right'} w-full font-semibold md:text-lg`}>{props.question}</div>
            <div className={`${isOpen?'block':'hidden'} ${locale === 'fr' ? 'text-left' : 'text-right'} text-sm md:text-lg font-light pt-2 productDetails`}>{props.answer}</div>
            </div>
